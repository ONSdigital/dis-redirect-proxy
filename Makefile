...
.PHONY: build
build: ## Builds binary of application code and stores in bin directory as dis-redirect-proxy
	go build -tags 'production' $(LDFLAGS) -o $(BINPATH)/dis-redirect-proxy
...
debug: ## Used to run code locally in debug mode
	go build -tags 'debug' $(LDFLAGS) -o $(BINPATH)/dis-redirect-proxy
	HUMAN_LOG=1 DEBUG=1 $(BINPATH)/dis-redirect-proxy
...
test-component: ## Runs component test suite
	go test -cover -coverpkg=github.com/ONSdigital/dis-redirect-proxy/... -component
